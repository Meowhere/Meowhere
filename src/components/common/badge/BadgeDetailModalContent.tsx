...
export default function BadgeDetailModalContent({ category, earnedAt }: Props) {
  ...
  return (
    <div className='flex flex-col items-center justify-center gap-[24px] text-center'>
      <motion.div className='w-[180px] h-[180px] flex items-center justify-center'>
        <BadgeGLBViewer glbPath={`/assets/badge/${badge.category}.glb`} />
      </motion.div>

      <div>
        <h2 className='text-[2.2rem] font-semibold text-gray-800 dark:text-gray-200'>
          {badge.title}
        </h2>
        <p className='text-md font-medium text-gray-600 dark:text-gray-400 pt-[6px]'>
          {badge.description}
        </p>
        <p className='text-xs text-gray-500 dark:text-gray-400 pt-[24px] gap-[8px]'>
          {earnedAt
